...
def load_all_model(
    unet_model_path=str(app_root() / "models" / "musetalkV15" / "unet.pth"),
    vae_type="sd-vae",
    unet_config=str(app_root() / "models" / "musetalkV15" / "musetalk.json"),
    device=None,
):
    vae = VAE(
        model_path = str(app_root() / "models" / vae_type),
    )
